...
云函数需要一个加密密钥用于 AES-256-CBC 加密。

1. 在云开发控制台 → **设置 → 环境变量**（或在云函数设置中配置）
2. 添加环境变量：

| 变量名 | 值 | 说明 |
|--------|-----|------|
| `ENCRYPTION_KEY` | 一个随机字符串（至少 32 字符） | AES 加密密钥，用于加密 openid、手机号、微信号 |
...
> **重要**：此密钥一旦设定并有数据写入后，不可更改，否则已加密数据将无法解密。请妥善保管。
...
```
错误: ENCRYPTION_KEY 环境变量未设置
```

1. 在云开发控制台确认环境变量 `ENCRYPTION_KEY` 已配置
2. 重新部署受影响的云函数（环境变量变更后需重新部署）
...
| 加密密钥 | 云函数环境变量 `ENCRYPTION_KEY` | AES-256 加密密钥 |
